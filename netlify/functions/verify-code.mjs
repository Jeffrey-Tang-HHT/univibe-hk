...
export default async (req) => {
  ...
  try {
    ...
    if (parts.length !== 2) return new Response(JSON.stringify({ error: "Invalid token" }), { status: 400, headers: h });

    const [receivedHmac, expiryStr] = parts;
    const expiry = parseInt(expiryStr, 10);

    if (Date.now() > expiry) return new Response(JSON.stringify({ error: "Code expired. Request a new one." }), { status: 400, headers: h });

    const expected = await createHmac(VERIFY_SECRET, `${email.toLowerCase()}:${code}:${expiry}`);
    if (receivedHmac !== expected) return new Response(JSON.stringify({ error: "Invalid verification code" }), { status: 400, headers: h });
